    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            if (strtolower($row['status']) == 'hidden') {
                ...
                echo '      <a href="?project='.$row['id'].'"><div class="project_view"><div class="project_title closed"><strike><span class="idtitle">#'.$row['id'].'</span>  '.substr($row['projectname'],0,150). '</strike> [' . $row['status'] . ']</div><br>', (($row['description'])) . '...</div></a>';
            } else {
                echo '      <a href="?project='.$row['id'].'"><div class="project_view"><div class="project_title"><span class="idtitle">#'.$row['id'].'</span>  '.substr($row['projectname'],0,150). ' [' . $row['status'] . ']</div><br>', (($row['description'])) . '...</div></a>';
            }
        }
    } else {
    } 
